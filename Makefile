...
# Use these settings to specify a custom Docker registry. docker.io is the public registry
...
test:
	${INFO} "Creating cache volume... " # Because docker-compose v2 does not create/destroy external volumes
	@ docker volume create --name cache

	${INFO} "Pulling latest images..." # This applies to images which are on docker hub (agent and db in our example) and not generated dynamically
	@ docker-compose -p $(DEV_PROJECT) -f $(DEV_COMPOSE_FILE) pull

	${INFO} "Building Images..."
	@ docker-compose -p $(DEV_PROJECT) -f $(DEV_COMPOSE_FILE) build --pull test # for services which are built dynamically
...
#	NOTE: We are not using "--rm" with builder, because we are copying 'wheelhouse' directory (containing artifacts) below. 
#         Also, we do not need to copy the artifacts if the build process fails, thus we are not using CHECK after copying artifacts
...
clean:
	...
	@ docker images -q -f dangling=true label=application=$(REPO_NAME) | xargs -I ARGS docker rm -f ARGS
...
# Introspect repository tags (Used by 'login:')
...
# Extract tag arguments. For. E.g. make tag 0.1 latest $(git rev-parse --short HEAD). Then this function will extract everything after 'make tag'
...
# @ suppresses the command used in the output, We can remove this '@' for troubleshooting
